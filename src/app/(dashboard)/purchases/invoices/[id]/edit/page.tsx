    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="rounded-2xl border p-5 text-left space-y-1" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>المجموع الفرعي: <span className="font-medium" style={{ color: "var(--fg)" }}>{Number(doc?.subtotal || 0).toLocaleString()} ر.س</span></p>
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>الضريبة: <span className="font-medium" style={{ color: "var(--fg)" }}>{Number(doc?.vat_total || 0).toLocaleString()} ر.س</span></p>
