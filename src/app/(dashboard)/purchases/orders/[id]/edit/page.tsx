...
export default function EditPurchaseOrderPage() {
  ...
  useEffect(() => {
    fetch(`/api/purchase-orders?id=${params.id}`).then((r) => r.json()).then((data) => {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      const res = await fetch(`/api/purchase-orders?id=${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
  ...
  const lines = (doc?.lines || doc?.order_lines || doc?.items || []) as Array<Record<string, unknown>>;

  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title={doc?.number ? `تعديل أمر شراء #${doc.number}` : "تعديل أمر شراء"} action={<Button variant="outline" onClick={() => router.back()}><ArrowRight className="h-4 w-4 ml-1" /> رجوع</Button>} />
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
          <h3 className="text-sm font-semibold mb-4" style={{ color: "var(--fg)" }}>معلومات أمر الشراء</h3>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="space-y-1.5"><Label className="text-xs font-medium">الحالة</Label>
              <select {...register("status")} className="flex h-10 w-full rounded-xl border px-3 text-sm" style={{ backgroundColor: "var(--surface)", borderColor: "var(--border)", color: "var(--fg)" }}>
                <option value="draft">مسودة</option><option value="sent">مرسل</option><option value="approved">معتمد</option><option value="cancelled">ملغي</option>
              </select>
            </div>
          </div>
        ...
        <div className="rounded-2xl border p-5" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
          ...
          {lines.length === 0 ? <p className="text-sm" style={{ color: "var(--text-muted)" }}>لا توجد بنود</p> : (
            <div className="overflow-x-auto"><table className="w-full text-sm">
              ...
              <tbody>{lines.map((l, i) => (<tr key={i} style={{ borderBottom: "1px solid var(--border)" }}><td className="py-2">{String(l.description || l.item_name || "—")}</td><td className="py-2 text-center">{String(l.qty ?? "—")}</td><td className="py-2 text-left">{Number(l.unit_price || 0).toLocaleString()}</td><td className="py-2 text-left">{Number(l.total || 0).toLocaleString()}</td></tr>))}</tbody>
            </table></div>
          )}
