        ...
        return (
          <RecordActionsMenu
            actions={[
              ...
              { type: "create-invoice", label: "إنشاء فاتورة مشتريات", icon: <FileText className="h-4 w-4" />, onClick: () => router.push(`/purchases/invoices/new?supplierId=${r.id}`) },
              { type: "create-order", label: "إنشاء أمر شراء", icon: <ShoppingCart className="h-4 w-4" />, onClick: () => router.push(`/purchases/orders/new?supplierId=${r.id}`) },
              { type: "receipt", label: "سند صرف", icon: <HandCoins className="h-4 w-4" />, onClick: () => router.push(`/cash/payments/new?supplierId=${r.id}`) },
              { type: "statement", label: "كشف حساب", icon: <ScrollText className="h-4 w-4" />, onClick: () => router.push(`/suppliers/${r.id}/statement`) },
