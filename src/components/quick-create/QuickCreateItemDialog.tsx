...
import { newId, type Item } from "@/lib/store";
import { useAudit } from "@/hooks/useAudit";
import { toast } from "sonner";

export function QuickCreateItemDialog({
  open, onOpenChange, onCreated,
}: { open: boolean; onOpenChange: (v: boolean) => void; onCreated?: (i: Item) => void }) {
  const { lang } = useI18n();
  const ar = lang === "ar";
  const audit = useAudit();
  const empty: Item = {
    id: newId(), name_ar: "", name_en: "", type: "service",
    sales_price: 0, taxable: true, vat_rate: 15, status: "active",
  };
  ...
  const handleSave = () => {
    ...
    if (form.sales_price < 0) {
      toast.error(ar ? "السعر لا يمكن أن يكون سالبًا" : "Price cannot be negative");
      return false;
    }
    ItemService.create(form);
    audit.log("item.created", "item", `إضافة منتج ${form.name_ar || form.name_en}`, `Created item ${form.name_en || form.name_ar}`, form.id);
    toast.success(ar ? "تم إنشاء المنتج" : "Item created");
    onCreated?.(form);
    reset();
    return true;
  };
  ...
  return (
    <QuickCreateDialog
      ...
      titleAr="إضافة منتج/خدمة سريع"
      titleEn="Quick add product / service"
      onSave={handleSave}
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v as Item["type"] })}>
            ...
            <SelectContent>
              ...
              <SelectItem value="non_stock">{ar ? "غير مخزني" : "Non-stock"}</SelectItem>
              <SelectItem value="stock">{ar ? "مخزني" : "Stock"}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          <Label>{ar ? "سعر البيع" : "Sales price"}</Label>
