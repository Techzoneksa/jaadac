...
export default function CreditNotePrintPage() {
  ...
  if (!data) return <div className="flex items-center justify-center min-h-screen"><p className="text-[#dc2626]">لم يتم العثور على إشعار الدائن</p></div>;
    ...
    <div className="min-h-screen bg-white" dir="rtl">
      ...
      <div className="max-w-[210mm] mx-auto p-8 print:p-4">
        <div className="border-b-2 border-[#0f172a] pb-4 mb-6">
          <h1 className="text-2xl font-bold text-[#0f172a]">إشعار دائن</h1>
          <p className="text-sm text-[#64748b]">رقم: {data.number}</p>
          <p className="text-sm text-[#64748b]">تاريخ: {data.date}</p>
          <p className="text-sm text-[#64748b]">العميل: {data.customer_name}</p>
        </div>
