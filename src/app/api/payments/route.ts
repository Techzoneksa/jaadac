...
type PayRow = Record<string, unknown> & { suppliers?: { name_ar: string } | null };
...
export async function GET(req: NextRequest) {
  return handle(async () => {
    ...
    const { data, error } = await supabase
      .from("payments")
      .select("*, suppliers!payments_supplier_id_fkey(name_ar)")
      .eq("tenant_id", user.id)
      .order("created_at", { ascending: false });

    if (error) return NextResponse.json({ error: error.message }, { status: 500 });

    const mapped = (data as PayRow[]).map((r) => ({
      ...r,
      supplier_name: r.suppliers?.name_ar || "",
    }));
