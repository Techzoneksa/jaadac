...
export default function EditReceiptPage() {
  ...
  useEffect(() => {
    fetch(`/api/receipts?id=${params.id}`).then((r) => r.json()).then((data) => {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      const res = await fetch(`/api/receipts?id=${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      const json = await res.json();
      if (!res.ok) { setError(json.error || "فشل الحفظ"); return; }
      router.push(`/cash/receipts/${params.id}`);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title={doc?.number ? `تعديل سند قبض #${doc.number}` : "تعديل سند قبض"} action={<Button variant="outline" onClick={() => router.back()}><ArrowRight className="h-4 w-4 ml-1" /> رجوع</Button>} />
