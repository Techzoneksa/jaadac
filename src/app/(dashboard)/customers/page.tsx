        ...
        return (
          <RecordActionsMenu
            actions={[
              ...
              { type: "create-invoice", label: "إنشاء فاتورة", icon: <FileText className="h-4 w-4" />, onClick: () => router.push(`/sales/invoices/new?customerId=${r.id}`) },
              { type: "create-quotation", label: "إنشاء عرض سعر", icon: <FileText className="h-4 w-4" />, onClick: () => router.push(`/sales/quotations/new?customerId=${r.id}`) },
              { type: "receipt", label: "سند قبض", icon: <HandCoins className="h-4 w-4" />, onClick: () => router.push(`/cash/receipts/new?customerId=${r.id}`) },
              { type: "statement", label: "كشف حساب", icon: <ScrollText className="h-4 w-4" />, onClick: () => router.push(`/customers/${r.id}/statement`) },
