...
export async function GET() {
  return handle(async () => {
    ...
    const { data, error } = await supabase.from("service_categories").select("*").eq("tenant_id", user.id).order("name");
...
export async function POST(req: Request) {
  return handle(async () => {
    ...
    const { data, error } = await supabase.from("service_categories").insert({ ...body, tenant_id: user.id }).select().single();
