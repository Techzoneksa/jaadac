...
type PODetail = Record<string, unknown> & {
...
export default function EditPurchaseOrderPage() {
  ...
  if (!po) return <p className="text-center py-8 text-[#dc2626]">لم يتم العثور على أمر الشراء</p>;

  return (
    <div>
      <PageHeader title={`تعديل أمر شراء #${po.number}`}
      ...
      <Card>
        <CardContent className="p-6">
          <div className="grid gap-4 sm:grid-cols-3 mb-4">
            <div><Label>الرقم</Label><p className="font-medium">{po.number}</p></div>
            <div><Label>التاريخ</Label><p className="font-medium">{po.date}</p></div>
            <div><Label>الحالة</Label><p className="font-medium">{po.status}</p></div>
          </div>
          ...
          <div className="text-left space-y-1">
            <p className="text-sm">المجموع الفرعي: {po.subtotal?.toLocaleString()} ر.س</p>
            <p className="text-sm">الضريبة: {po.vat_total?.toLocaleString()} ر.س</p>
            <p className="text-lg font-bold">الإجمالي: {po.total?.toLocaleString()} ر.س</p>
          </div>
          {po.notes && <p className="mt-4 text-sm text-[#64748b]">{po.notes}</p>}
        </CardContent>
      </Card>
    </div>
  );
