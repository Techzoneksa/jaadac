...
interface DebitNoteData {
  ...
  debit_note_lines?: Array<{ description?: string; qty?: number; unit_price?: number; vat_rate?: number; total?: number }>;
}
...
export default function DebitNotePrintPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/debit-notes?id=${params.id}`).then((r) => r.json()),
  ...
  if (!data) return <div className="flex items-center justify-center min-h-screen"><p className="text-danger">لم يتم العثور على إشعار المدين</p></div>;
      ...
      <div className="print-document">
        <div className="flex justify-between items-start border-b-2 border-[#0f172a] pb-4 mb-6">
          ...
          <div className="text-left">
            <h2 className="text-lg font-bold">إشعار مدين</h2>
        ...
        <table className="w-full text-sm mb-6">
          ...
          <tbody>
            {(data.debit_note_lines || []).map((line, i) => (
