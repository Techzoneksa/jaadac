...
interface FormData { date: string; status: string; notes: string; customer_name: string; }
...
export default function EditCreditNotePage() {
  ...
  const { register, handleSubmit, reset } = useForm<FormData>({ defaultValues: { date: "", status: "draft", notes: "", customer_name: "" } });

  useEffect(() => {
    fetch(`/api/credit-notes?id=${params.id}`).then((r) => r.json()).then((data) => {
      if (data.error) { setError(data.error); setLoading(false); return; }
      setDoc(data); reset({ date: data.date || "", status: data.status || "draft", notes: data.notes || "", customer_name: data.customer_name || "" }); setLoading(false);
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      const res = await fetch(`/api/credit-notes?id=${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      const json = await res.json();
      if (!res.ok) { setError(json.error || "فشل الحفظ"); return; }
      router.push(`/sales/credit-notes/${params.id}`);
  ...
  const lines = (doc?.lines || doc?.credit_note_lines || doc?.invoice_lines || []) as Array<Record<string, unknown>>;

  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title={doc?.number ? `تعديل إشعار دائن #${doc.number}` : "تعديل إشعار دائن"} action={<Button variant="outline" onClick={() => router.back()}><ArrowRight className="h-4 w-4 ml-1" /> رجوع</Button>} />
