...
export async function GET() {
  return handle(async () => {
    ...
    const { data, error } = await supabase.from("inventory_branches").select("*").eq("tenant_id", user.id).order("name");
...
export async function POST(req: Request) {
  return handle(async () => {
    ...
    const { data, error } = await supabase.from("inventory_branches").insert({ ...body, tenant_id: user.id }).select().single();
